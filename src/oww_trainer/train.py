...
if __name__ == '__main__':
    ...
    if args.train_model is True:
        ...
        X_val_fp_raw = np.load(config["false_positive_validation_data_path"])
        X_val_fp_win = np.array([X_val_fp_raw[i:i+input_shape[0]] for i in range(0, X_val_fp_raw.shape[0]-input_shape[0], 1)])
        X_val_fp_labels = np.zeros(X_val_fp_win.shape[0]).astype(np.float32)
        X_val_fp = torch.utils.data.DataLoader(
            torch.utils.data.TensorDataset(torch.from_numpy(X_val_fp_win), torch.from_numpy(X_val_fp_labels)),
            batch_size=8192
        )
        del X_val_fp_raw, X_val_fp_win, X_val_fp_labels
        ...
        X_val = torch.utils.data.DataLoader(
            ...
            batch_size=min(len(labels), 8192)
        )
        del X_val_combined, labels
